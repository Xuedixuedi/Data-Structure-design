...
​		对于两个链表，想要求他们的交集，只需要对链表进行遍历并比较其中元素即可。对于本题所给情况：两个链表都是非降序链表，则可以在时间复杂度和空间复杂度上面对普通链表求交集的方法上有所优化和改进。

### 1.2 项目功能要求

​		该项目的核心函数，需要分析两个链表的元素，提取其中相同的元素储存入一个新的链表。这个链表就是我们要求得的两个链表的交集。为了使项目的普适性更强，使项目操作更安全，本项目将操作链表和节点的方法进行了封装。
